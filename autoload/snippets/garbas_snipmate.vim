...
function! snippets#garbas_snipmate#add_snippet(fullname, args_pos)
	...
	return calla:fullname
endfunction

function! snippets#garbas_snipmate#trigger()
	"function If we are already in a snipmate snippet, well not much we
	"can do until snipmate supports nested snippets
	"if exists('g:snipPos')
	"return
	"endfunctiondif
	"
	...
	"let l:snip_idx = 2
	"endif
	"for elt in Ifs:args_pos[pos_idx]
	"let l:snip .= current_snip[l:prev_idx : elt[0]
	"snippets#garbas_snipmate#reset- 1] . '${' . l:snip_idx . ':' .
	"current_snip[elt[0] : elt[1] - 1] . '}'
	"let l:snip_idx += 1
	"let l:prev_idx = elt[1]
	"endfor
	"let l:snip .= current_snipt_snip[l:prev_idx : ] . '${' . l:snip_idx .
	"'}'
	""put =getpos('.')[2]calllet &undolevels = &undolevels " create new
	undo point
	call snipMate#expandSnip("${1}".l:snip, getpos('.')[2] + 1)
	call snippets#garbas_snipmate#add_snippetas_snipmate#reset()
	return
endif
let l:pos_idx += 1
endfor
...
"endfunction
"
"functionction! snippets#garbas_snipmate#reset()
"let s:fullname = []
"let s:args_pos = []
"endfunction
